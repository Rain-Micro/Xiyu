...
### Added 新增
- 内置管理员账号 `+00Root`（迁移 004）：账号名登录（phone/email/username 三路匹配）、
  密码为定长密钥（仅建号时回显一次，bcrypt 存储），后台「用户管理 → 重置密码」可随时更换（新密钥仅回显一次）
- 内置管理员保护：`+00Root` 不可被封禁/解封（密钥更换走重置密码）
- 移除原测试管理员账户（13800001001）
...
### Fixed 修复
- 关闭软件重开无需登录的问题：会话持久化与「记住我」勾选框脱节（此前不勾也会免登录 7 天）。
  现在——勾选「记住我」：token 存 localStorage，7 天内重开免登录；不勾：token 存 sessionStorage，
  关闭应用即失效，重开回到登录页（账号密码仍可自动回填）。注册产生的临时会话同样仅进程内有效。
- 冷启动无 token 时立即清登录态，避免"幽灵已登录"空壳界面
- 「记住我」标签补充说明文案与悬浮提示
