...
import { synthesizeSpeech, listVoices, configureOperit, getOperitConfig } from '../services/voiceService';
...
// 动态配置 Operit（运行时）
router.post('/operit/config', (req: Request, res: Response) => {
    const { apiUrl, token } = req.body;
    if (typeof apiUrl !== 'string' || typeof token !== 'string') {
        return res.status(400).json({ error: '参数格式错误，需要 apiUrl(string) 和 token(string)' });
    }
    configureOperit(apiUrl, token);
    res.json({ success: true, config: getOperitConfig() });
});
